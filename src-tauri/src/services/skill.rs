...
impl SkillService {
    ...
    pub async fn check_updates(&self, db: &Arc<Database>) -> Result<Vec<SkillUpdateInfo>> {
        ...
        for ((owner, name, branch), group_skills) in &repo_groups {
            ...
            for skill in group_skills {
                // 上游 #6381：三级匹配（readme_url doc_path → 末级目录名 → 唯一
                // metadata name），支持 skillId 与目录名不一致。本地保留删除检测
                // 语义：匹配或源目录解析任一失败 → SkillDeleted（用户可见原因），
                // 不静默跳过。
                ...
                // 存量 readme_url 纠偏（#6111 前的坏链接）：远端扫描项的
                // readme_url 由真实相对路径（strip_prefix）构建，是权威事实，
                // 直接对齐它——不用 directory 拼接（repo 根场景 directory 是
                // repo.name 占位符，拼出假路径 repo/SKILL.md 会让下轮匹配
                // 三级全空）。哈希一致无更新条目的 skill 也要修正，否则箭头
                // 跳转永远 404。
                if let Some(authoritative) = remote_match
                    .and_then(|rs| rs.readme_url.as_deref())
                    .filter(|url| Some(*url) != skill.readme_url.as_deref())
                {
                    log::info!("修正 skill {} 的文档链接: {authoritative}", skill.id);
                    let _ = db.update_skill_readme_url(&skill.id, authoritative);
                }
    ...
    pub async fn update_skill(&self, db: &Arc<Database>, skill_id: &str) -> Result<InstalledSkill> {
        ...
        let remote_match = remote_match.ok_or_else(|| {
            anyhow!(format_skill_error(
                "SKILL_DIR_NOT_FOUND",
        ...
        // 更新 readme_url（#6111 系）：远端真实目录权威——choose_doc_path 以
        // canonicalize 出的 resolved_doc_path 为第一优先级，不从旧 readme_url
        // 提取坏路径（存量坏链接只做规范化兑底）。
        let doc_path = Self::choose_doc_path(
            resolved_doc_path,
            skill.readme_url.as_deref(),
